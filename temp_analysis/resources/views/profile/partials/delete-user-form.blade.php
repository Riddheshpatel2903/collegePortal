<section class="space-y-6">
    ...
    <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('profile.destroy') }}"
            class="p-10 bg-white dark:bg-gray-900 rounded-[2.5rem] border border-gray-100 dark:border-gray-800">
            @csrf
            @method('delete')

            <h2 class="text-2xl font-black text-gray-800 dark:text-white uppercase tracking-tighter mb-4">
            ...
            <div class="mb-8">
                <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-2">Access Token
                    Verification</label>
                <x-text-input id="password" name="password" type="password"
                    class="block w-full bg-gray-50/50 dark:bg-gray-900/50 border-none rounded-2xl py-3.5 px-5 font-bold shadow-inner focus:ring-2 focus:ring-rose-500/30 transition-all dark:text-white"
            ...
            <div class="flex justify-end gap-4">
                <button type="button" x-on:click="$dispatch('close')"
                    class="px-6 py-3 bg-gray-100 dark:bg-gray-800 text-gray-500 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-gray-200 transition-all">
                    {{ __('Abort Mission') }}
                </button>
