...
@section('content')
    <div class="flex items-center justify-between mb-8">
        <div>
            <h2 class="text-2xl font-extrabold text-slate-800 tracking-tight">New Notice</h2>
            <p class="text-sm text-slate-400 mt-1">Publish an announcement to the college notice board.</p>
        </div>
            ...
            <form action="{{ route('admin.notices.store') }}" method="POST" class="space-y-6">
                ...
                <div class="flex items-center gap-3 mb-6 pb-6 border-b border-slate-100">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400">Fill in the announcement information.</p>
                    </div>
                </div>
                <div>
                    <label
                ...
                <div>
                    <label class="block text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Target
                        Audience</label>
                    <select name="target_role"
                        class="w-full border border-slate-200 rounded-xl py-3 px-4 text-sm text-slate-700 bg-slate-50/50 focus:ring-2 focus:ring-violet-500/20 focus:border-violet-300 transition-all">
                        <option value="all" {{ old('target_role') == 'all' ? 'selected' : '' }}>All</option>
                        <option value="student" {{ old('target_role') == 'student' ? 'selected' : '' }}>Students Only</option>
                        <option value="teacher" {{ old('target_role') == 'teacher' ? 'selected' : '' }}>Faculty Only</option>
                    </select>
                </div>
                <div>
                    <label
                    ...
                    <textarea name="content" rows="6"
                        ...
                        placeholder="Write your notice here..." required>{{ old('content') }}</textarea>
                </div>
                <div class="flex justify-end gap-3 pt-6 border-t border-slate-100">
                    ...
                    <button type="submit"
                        class="px-8 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-violet-600 to-purple-600 rounded-xl hover:shadow-lg hover:shadow-violet-500/25 transition-all">Publish
                        Notice</button>
                </div>
            </form>
        </div>
