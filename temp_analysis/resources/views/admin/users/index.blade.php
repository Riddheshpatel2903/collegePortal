...
@section('content')
    <div class="space-y-8 animate-fade-in">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
                <h2 class="text-3xl font-black text-slate-900 tracking-tight mb-2">Registry Control</h2>
                <div class="flex items-center gap-3">
                    <x-badge type="info" class="!bg-violet-50 !text-violet-600 border-none font-black text-[10px] uppercase tracking-widest">
            ...
            <div class="flex flex-wrap items-center gap-4">
                ...
                <div class="relative group">
                    <form action="{{ route('admin.users.index') }}" method="GET" id="filterForm" class="flex flex-col sm:flex-row items-center gap-3">
                        {{-- Global Search Bar --}}
                        ...
                        <select name="role" onchange="document.getElementById('filterForm').submit()" 
                            ...
                            <option value="teacher" {{ request('role') == 'teacher' ? 'selected' : '' }}>Faculty Members</option>
                            <option value="student" {{ request('role') == 'student' ? 'selected' : '' }}>Active Students</option>
                            <option value="accountant" {{ request('role') == 'accountant' ? 'selected' : '' }}>Financial Officers</option>
                        </select>
                       </form>
                </div>

                <x-button variant="primary" href="{{ route('admin.users.create') }}" icon="bi-person-plus-fill" class="!py-3 !px-6 shadow-xl shadow-violet-200">
                    Register Identity
                </x-button>
            </div>
        </div>
                    ...
                    <tr class="group hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="py-5">
                            <div class="flex items-center gap-4">
                                <div class="relative">
                                    <img src="https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&background={{ $user->role == 'admin' ? 'f59e0b' : ($user->role == 'teacher' ? '7c3aed' : ($user->role == 'accountant' ? '10b981' : '0ea5e9')) }}&color=fff&bold=true" 
                                        class="h-12 w-12 rounded-2xl shadow-sm ring-2 ring-white group-hover:scale-105 transition-transform" alt="">
                                </div>
                                <div class="min-w-0">
                                    <div class="text-sm font-black text-slate-800 tracking-tight truncate">{{ $user->name }}</div>
                                    <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">ID: #SYS{{ str_pad($user->id, 4, '0', STR_PAD_LEFT) }}</div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td>
                            @php
                                ...
                                ][$user->role] ?? ['bg' => 'bg-slate-50', 'text' => 'text-slate-600', 'label' => 'Guest'];
                            @endphp
                            <x-badge type="info" class="!{{ $roleConfig['bg'] }} !{{ $roleConfig['text'] }} border-none font-black text-[9px] uppercase tracking-widest px-3 py-1.5 rounded-xl">
                                {{ $roleConfig['label'] }}
                            </x-badge>
                        </td>
                            ...
                            <div class="text-sm text-slate-600">
                                @if($user->role === 'student')
                                    ...
                                    <div class="text-[10px] text-slate-400 uppercase tracking-widest">
                                        {{ $user->student->gtu_enrollment_no ?? 'GTU-N/A' }} | Sem {{ $user->student->current_semester_number ?? 'N/A' }}
                                    </div>
                                @elseif($user->role === 'teacher')
                                    <div class="font-bold">{{ $user->teacher->department->name ?? 'Faculty' }}</div>
                                    <div class="text-[10px] text-slate-400 uppercase tracking-widest">{{ $user->teacher->qualification ?? 'Professor' }}</div>
                    ...
                    <tr>
                        <td colspan="5">
                            <div class="py-20 text-center">
                                <div class="h-20 w-20 bg-slate-50 rounded-full flex items-center justify-center text-slate-300 text-3xl mx-auto mb-6">
                                    <i class="bi bi-people-fill"></i>
                                </div>
                                <h3 class="text-lg font-black text-slate-800 tracking-tight">No identities detected</h3>
                                <p class="text-sm font-bold text-slate-400 uppercase tracking-widest mt-2">Adjust your filters or register a new user</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
