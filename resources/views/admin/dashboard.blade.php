...
@section('content')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        @foreach([
            ['route' => 'admin.students.index', 'label' => 'Total Students', 'value' => number_format($studentCount), 'icon' => 'bi-people-fill', 'tone' => 'violet'],
            ['route' => 'admin.teachers.index', 'label' => 'Total Teachers', 'value' => number_format($teacherCount), 'icon' => 'bi-mortarboard-fill', 'tone' => 'teal'],
            ['route' => 'admin.courses.index', 'label' => 'Active Courses', 'value' => number_format($courseCount), 'icon' => 'bi-journal-bookmark-fill', 'tone' => 'amber'],
            ['route' => 'admin.fees.index', 'label' => 'Fee Collection', 'value' => ' ' . number_format($feeTotal), 'icon' => 'bi-currency-rupee', 'tone' => 'rose'],
        ] as $stat)
            @canPage($stat['route'])
                        ...
                        <button type="button"
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center justify-between gap-2 mb-1">
                                    <h4 class="text-sm font-semibold text-slate-700 group-hover:text-violet-600 transition-colors truncate">{{ $notice->title }}</h4>
