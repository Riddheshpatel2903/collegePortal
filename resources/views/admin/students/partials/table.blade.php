@if($studentsPaginated->isEmpty())
    <div class="aero-card py-32 text-center border-dashed border-2 border-slate-200/60 bg-slate-50/30">
        <div class="relative h-24 w-24 mx-auto mb-8">
            ...
            <div
                class="relative h-full w-full rounded-full bg-white shadow-2xl flex items-center justify-center text-4xl text-slate-400">
                <i class="bi bi-search"></i>
            </div>
        </div>
        <h3 class="text-2xl font-black text-slate-800 tracking-tight mb-2">No Records Detected</h3>
        <p class="text-slate-400 font-medium max-w-sm mx-auto">We couldn't find any students matching those parameters. Try
            resetting your search filters.</p>
    </div>
@else
    @forelse($studentsGrouped as $semesterName => $students)
        <div class="mb-12 animate-fade-in group-container last:mb-0">
            <div class="flex items-center gap-4 mb-6 px-4">
                <div
                    class="h-10 w-1.5 bg-gradient-to-b from-violet-600 to-indigo-600 rounded-full shadow-lg shadow-violet-500/20">
                </div>
                <div>
            ...
            <x-table :headers="['GTU ID', 'Student Profile', 'Domain & Degree', 'Intake', 'Access', 'Nexus']">
                @foreach($students as $student)
                    <tr class="group/row" data-student-id="{{ $student->id }}">
                            ...
                            <div class="flex items-center gap-4">
                                <div class="relative">
                                    <img class="h-10 w-10 rounded-xl"
                                        ...
                                        alt="">
                                    <span class="status-dot absolute -bottom-1 -right-1 h-3 w-3 border-2 border-white rounded-full
                        ...
                        <td>
                            <div class="text-xs font-bold text-slate-700 leading-none mb-1">{{ $student->course?->name ?? '-' }}
                            </div>
                            <div class="text-[9px] text-slate-400 font-black uppercase tracking-widest">
                                {{ $student->course?->department?->name ?? '-' }}</div>
                        </td>
                        ...
                        <td>
                            <div class="flex justify-between gap-2">
                                <a href="{{ route('admin.students.edit', $student->id) }}" class="action-btn action-btn-edit"
                                    title="Edit Student">
                                ...
                                <button type="button" data-action="toggle"
                                    ...
                                    title="{{ $student->is_active ? 'Deactivate Student' : 'Activate Student' }}" class="action-btn toggle-btn {{ $student->is_active
                            ? 'bg-emerald-50 text-emerald-600 border border-emerald-200 hover:bg-emerald-500 hover:text-white hover:border-emerald-500'
                            : 'bg-amber-50 text-amber-500 border border-amber-200 hover:bg-amber-500 hover:text-white hover:border-amber-500' }}
                                                transition-all duration-200">
                                    <i
                                        class="bi {{ $student->is_active ? 'bi-toggle-on' : 'bi-toggle-off' }} text-lg leading-none"></i>
                                </button>
                                ...
                                <button type="button" data-action="delete"
                                    ...
                                    title="Delete Student" class="action-btn action-btn-delete">
...
@endif
    ...
    @if($studentsGrouped->isNotEmpty())
        ...
        <div
            class="mt-12 aero-card p-6 border border-white/60 shadow-2xl shadow-slate-200/30 flex flex-col md:flex-row items-center justify-between gap-6 min-w-0 max-w-full">
            <div class="flex items-center gap-4 text-xs font-black text-slate-400 uppercase tracking-widest flex-shrink-0">
                <span
                    class="h-10 w-10 rounded-xl bg-slate-100 flex items-center justify-center text-slate-600 shadow-inner">
