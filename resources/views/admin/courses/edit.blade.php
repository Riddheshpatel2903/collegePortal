...
@section('content')
    <div class="flex items-center justify-between mb-8">
        <h2 class="text-2xl font-extrabold text-slate-800 tracking-tight">Edit Course</h2>
    ...
    <div class="glass-card p-8 max-w-2xl">
        <form method="POST" action="{{ route('admin.courses.update', $course->id) }}" class="space-y-5">
            @csrf
            @method('PUT')
            <div>
            ...
            <div>
                <label class="block text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Duration
                    (Years)</label>
                <input type="number" name="duration_years" value="{{ old('duration_years', $course->duration_years) }}"
                    required
                    class="w-full bg-white border border-slate-200 rounded-xl py-2.5 px-4 text-sm text-slate-700 focus:ring-2 focus:ring-violet-500/20 focus:border-violet-300 transition-all">
            </div>
            <button type="submit"
